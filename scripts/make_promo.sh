...
card(){ local o=$1 zh=$2 en=$3 sub=$4; wood "$TMP/_b.png"
  convert "$TMP/_b.png" -font "$FB" -gravity center \
    -fill "$AMBER_SH" -pointsize 96 -annotate +4+4 "$zh" -fill "$AMBER" -pointsize 96 -annotate +0-10 "$zh" \
    -font "$FR" -fill "$TEXT" -pointsize 46 -annotate +0+80 "$en" \
    -fill "$SUBTLE" -pointsize 30 -annotate +0+150 "$sub" "$o"; }
...
dcard(){ local o=$1 scene=$2 zh=$3 en=$4; wood "$TMP/_b.png"
  ...
  convert -background none -fill "$GREEN" -font "$FB" -pointsize 52 -size $((W-420))x -gravity center caption:"$zh" "$TMP/_z.png"
  convert "$TMP/_b.png" "$TMP/_p.png" -gravity center -composite \
    -font "$FB" -fill "$BRICK" -gravity north -pointsize 32 -annotate +0+96 "— $scene —" \
    "$TMP/_z.png" -gravity center -geometry +0+10 -composite \
    -font "$FR" -fill "$PARCH_BD" -gravity south -pointsize 24 -annotate +0+100 "$en" "$o"; }
...
slide_frame(){ local o=$1 sc=$2 cap=$3; wood "$TMP/_b.png"
  ...
  convert "$TMP/_b.png" "$TMP/_f.png" -gravity north -geometry +0+44 -composite \
    -font "$FR" -fill "$TEXT" -gravity south -pointsize 34 -annotate +0+34 "$cap" "$o"; }

# F8 對比:左遊戲綠中文 | 右淡褐英文
split_ba(){ local o=$1 zh=$2 en=$3; wood "$TMP/_b.png"
  convert -background none -fill "$GREEN_L" -font "$FB" -pointsize 42 -size 500x -gravity center caption:"$zh" "$TMP/_l.png"
  convert -background none -fill "$SUBTLE"  -font "$FR" -pointsize 40 -size 500x -gravity center caption:"$en" "$TMP/_r.png"
  convert "$TMP/_b.png" \
    "$TMP/_l.png" -gravity west -geometry +80+20 -composite \
    "$TMP/_r.png" -gravity east -geometry +80+20 -composite \
    -font "$FB" -fill "$AMBER" -gravity center -pointsize 60 -annotate +0-20 "F8" \
    -font "$FR" -fill "$AMBER" -gravity center -pointsize 26 -annotate +0+40 "◀ ▶" \
    -font "$FR" -fill "$SUBTLE" -gravity north -pointsize 30 -annotate +0+70 "一鍵切換 中 / 英字幕(語音維持英語)" "$o"; }
...
capbar(){ local o=$1 cap=$2
  convert -size ${W}x${H} xc:none \
    ...
    -font "$FR" -fill "$TEXT" -gravity south -pointsize 34 -annotate +0+26 "$cap" "$o"; }
...
dcard "$TMP/02.png" "問題" "英文 CD 版沒有完整字幕——嘴巴在動,一個字都看不到。" "English CD has no full subtitles."
dcard "$TMP/03.png" "解法" "floppy 完整文字 × CD 英語配音,融合。" "Floppy text + CD voice, fused."
...
dcard "$TMP/09.png" "巨魔橋" "你走運了,我正好是個到處推銷滿足的業務員。" "Luckily, I'm a travelling satisfaction salesman."
